...
$(document).ready(function() {
    ...
    if (bc_regex.test(this.value)) {
      $.ajax({
        ...
        success : function(data) {
          if ( v_a = data.valid) {
            $("#g_id").toggleClass('invis', false);
            $("#g_id").focus();
          } else {
            $('#new_attendee').dialog('open');
          }
          $("#a_id").attr('readonly', data.valid);
  ...
  $('#x_game').click(function() {
    //$("#g_id").attr('readonly', false); not needed currently
    $("#g_id").val('');
    $("#g_label").text('');
    v_g = false;
  });
  ...
  $("#new_attendee").dialog({
    ...
    buttons : {
      "Add Attendee" : function() {
        data = $('#new_attendee > input[type="text"], #a_id').serializeArray();
        $.ajax({
